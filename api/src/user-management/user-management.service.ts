...
export class UserManagementService {
  ...
  async create(dto: CreateUserManagementDto, avatar: Express.Multer.File) {
    try {
      ...
      const userExistence = await this.userModel.findOne({ account: dto.account })

      if(userExistence) {
        return new ResponseCommon(HttpStatus.CONFLICT, false, "USER_ALREADY_EXISTED", userExistence)
      }
      ...
      const saveToS3Params: PutObjectCommandInput = {
        Bucket: s3Configs.bucket,
        Key: `users/${newUser.id}/avatar/${formatFileName(avatar.originalname)}`,
        Body: avatar.buffer
      }

      await this.s3.putObject(saveToS3Params).catch((rejectReason) => {
        return new ResponseCommon(HttpStatus.INTERNAL_SERVER_ERROR, false, "UPLOAD_TO_S3_FAILED", rejectReason)
      })
      ...
      if (saveUser) {
        ...
        return new ResponseCommon(HttpStatus.INTERNAL_SERVER_ERROR, true, "INTERNAL_SERVER_ERROR")
      }
    } catch (error) {
  ...
  async findAll() {
    try {
      const findResult = await this.userModel.find().exec()
  ...
  async removeAllUser() {
    ...
    return await this.userModel.deleteMany({ _id: allUser.map(user => user.id) })
  }
}
